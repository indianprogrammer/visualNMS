...
const { signToken, requireAuth, requireRole, requirePerm, permissions: permCatalog } = require('./auth');
...
router.post('/auth/login', async (req, res) => {
  ...
  res.json({
    ...
    user: { id: user.id, username: user.username, role: user.role, permissions: user.permissions || { pages: [], ops: [] }, created_at: user.created_at }
  });
});

router.get('/auth/permissions', requireAuth, (req, res) => {
  res.json({ pages: permCatalog.PAGES, ops: permCatalog.OPS, templates: permCatalog.TEMPLATES });
});
...
// ── Users (admin only) ──
const VALID_ROLES = ['readonly', 'readwrite', 'admin'];
function userPublic(u) {
  return { id: u.id, username: u.username, role: u.role, permissions: u.permissions || { pages: [], ops: [] }, created_at: u.created_at };
}

router.get('/users', requireAuth, requireRole('admin'), async (req, res) => {
  const users = await db.find('users', {}, { sort: { username: 1 } });
...
router.post('/users', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  if (!/^[a-zA-Z0-9._-]{3,32}$/.test(username)) return res.status(400).json({ error: 'Username must be 3-32 chars (letters, digits, . _ -)' });
  if (password.length < 4) return res.status(400).json({ error: 'Password must be at least 4 characters' });
  const role = VALID_ROLES.indexOf(b.role) >= 0 ? b.role : 'readonly';
  try {
    const existing = await db.findOne('users', { username });
    if (existing) return res.status(409).json({ error: 'Username already exists' });
    const hash = await bcrypt.hash(password, 10);
    const permissions = permCatalog.sanitize(b.permissions);
    // A custom non-template config must still grant the settings page if
    // profile/settings ops are present; otherwise it can't reach Settings.
    const id = await db.ins('users', { username, password_hash: hash, role, permissions });
    await db.addEvent(null, 'auth', `User created: ${username} (${role})`, 'system', 'info');
...
router.put('/users/:id', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  const isSelf = id === db.id(req.user.id);
  const set = {};
  if (b.role !== undefined) {
    if (isSelf) return res.status(400).json({ error: 'You cannot change your own role' });
    const role = VALID_ROLES.indexOf(b.role) >= 0 ? b.role : null;
    if (role === null) return res.status(400).json({ error: 'Invalid role' });
    if (target.role === 'admin' && role !== 'admin') {
      const admins = await db.find('users', { role: 'admin' }, { projection: { id: 1 } });
      if (admins.length <= 1) return res.status(400).json({ error: 'Cannot demote the last admin' });
    }
    set.role = role;
  }
  if (b.permissions !== undefined) {
    if (isSelf) return res.status(400).json({ error: 'You cannot change your own permissions' });
    set.permissions = permCatalog.sanitize(b.permissions);
  }
  ...
  await db.updateOne('users', { id }, { $set: set });
  if (set.password_hash) await db.addEvent(null, 'auth', `Password changed for: ${target.username}`, 'system', 'info');
  res.json({ message: 'Updated' });
});

router.delete('/users/:id', requireAuth, requireRole('admin'), async (req, res) => {
  const id = db.id(req.params.id);
  if (id === db.id(req.user.id)) return res.status(400).json({ error: 'You cannot delete your own account' });
  const target = await db.findOne('users', { id });
  if (!target) return res.status(404).json({ error: 'User not found' });
  ...
  await db.addEvent(null, 'auth', `User deleted: ${target.username}`, 'system', 'info');
  res.json({ message: 'Deleted' });
});

// ── Devices ──
router.get('/devices', requireAuth, requirePerm('pages', 'devices'), async (req, res) => {
  res.json(await db.allDevices());
});

router.get('/devices/:id', requireAuth, requirePerm('pages', 'devices'), async (req, res) => {
  const d = await db.devById(req.params.id);
  if (!d) return res.status(404).json({ error: 'Not found' });
  res.json(d);
});

router.post('/devices', requireAuth, requirePerm('ops', 'device:add'), async (req, res) => {
  const b = req.body || {};
...
router.put('/devices/:id', requireAuth, requirePerm('ops', 'device:edit'), async (req, res) => {
  const b = req.body || {};
  if (!b.name || !b.ip_address) return res.status(400).json({ error: 'name and ip_address required' });
  const id = db.id(req.params.id);
  try {
...
router.delete('/devices/:id', requireAuth, requirePerm('ops', 'device:delete'), async (req, res) => {
  const id = db.id(req.params.id);
...
router.get('/devices/:id/metrics', requireAuth, requirePerm('pages', 'devices'), async (req, res) => {
...
router.get('/devices/:id/interfaces', requireAuth, requirePerm('pages', 'devices'), async (req, res) => {
  res.json(await db.interfacesByDev(req.params.id));
});

router.get('/devices/:id/ping', requireAuth, requirePerm('ops', 'device:test'), async (req, res) => {
  const d = await db.devById(req.params.id);
  if (!d) return res.status(404).json({ error: 'Not found' });
  res.json(await pingPoller.pingHost(d.ip_address));
});

router.get('/devices/:id/snmp', requireAuth, requirePerm('ops', 'device:test'), async (req, res) => {
  const d = await db.devById(req.params.id);
  if (!d) return res.status(404).json({ error: 'Not found' });
  try {
...
router.post('/devices/:id/refresh', requireAuth, requirePerm('ops', 'device:refresh'), async (req, res) => {
  const d = await db.devById(req.params.id);
  if (!d) return res.status(404).json({ error: 'Not found' });
  try {
...
// ── Maps ──
router.get('/maps', requireAuth, requirePerm('pages', 'topology'), async (req, res) => {
  res.json(await db.find('maps', {}, { sort: { title: 1 } }));
});

router.post('/maps', requireAuth, requirePerm('ops', 'map:add'), async (req, res) => {
...
router.get('/maps/:id', requireAuth, requirePerm('pages', 'topology'), async (req, res) => {
...
router.put('/maps/:id', requireAuth, requirePerm('ops', 'map:edit'), async (req, res) => {
  const id = db.id(req.params.id);
...
router.delete('/maps/:id', requireAuth, requirePerm('ops', 'map:delete'), async (req, res) => {
  const id = db.id(req.params.id);
...
router.post('/maps/:id/nodes', requireAuth, requirePerm('ops', 'node:add'), async (req, res) => {
  const b = req.body || {};
  const mapId = db.id(req.params.id);
...
router.put('/maps/:mapId/nodes/:nodeId', requireAuth, requirePerm('ops', 'node:move'), async (req, res) => {
  const b = req.body || {};
...
router.delete('/maps/:mapId/nodes/:nodeId', requireAuth, requirePerm('ops', 'node:delete'), async (req, res) => {
  const mapId = db.id(req.params.mapId);
  const nodeId = db.id(req.params.nodeId);
...
router.post('/maps/:id/links', requireAuth, requirePerm('ops', 'link:add'), async (req, res) => {
  const b = req.body || {};
...
router.delete('/maps/:mapId/links/:linkId', requireAuth, requirePerm('ops', 'link:delete'), async (req, res) => {
  const mapId = db.id(req.params.mapId);
...
router.put('/maps/:mapId/links/:linkId', requireAuth, requirePerm('ops', 'link:edit'), async (req, res) => {
  const b = req.body || {};
  const mapId = db.id(req.params.mapId);
  const linkId = db.id(req.params.linkId);
...
// ── Alerts ──
router.get('/alerts', requireAuth, requirePerm('pages', 'alerts'), async (req, res) => {
  const q = req.query || {};
  let filter = {};
...
router.post('/alerts/:id/acknowledge', requireAuth, requirePerm('ops', 'alert:ack'), async (req, res) => {
  await db.ackAlert(db.id(req.params.id), req.user.id);
  res.json({ message: 'Acknowledged' });
});

router.post('/alerts/:id/resolve', requireAuth, requirePerm('ops', 'alert:resolve'), async (req, res) => {
  await db.resolveAlert(db.id(req.params.id));
  res.json({ message: 'Resolved' });
});

// ── Alert Rules ──
router.get('/alert-rules', requireAuth, requirePerm('pages', 'alerts'), async (req, res) => {
  res.json(await db.allRules(false));
});

router.post('/alert-rules', requireAuth, requirePerm('ops', 'rules:manage'), async (req, res) => {
  const b = req.body || {};
...
router.delete('/alert-rules/:id', requireAuth, requirePerm('ops', 'rules:manage'), async (req, res) => {
  await db.deleteOne('alert_rules', { id: db.id(req.params.id) });
  res.json({ message: 'Deleted' });
});

// ── SNMP Profiles ──
router.get('/snmp-profiles', requireAuth, requirePerm('pages', 'settings'), async (req, res) => {
  res.json(await db.find('snmp_profiles', {}, { sort: { name: 1 } }));
});

router.post('/snmp-profiles', requireAuth, requirePerm('ops', 'profile:manage'), async (req, res) => {
  const b = req.body || {};
...
router.put('/snmp-profiles/:id', requireAuth, requirePerm('ops', 'profile:manage'), async (req, res) => {
  const b = req.body || {};
  if (!b.name) return res.status(400).json({ error: 'name required' });
  try {
...
router.delete('/snmp-profiles/:id', requireAuth, requirePerm('ops', 'profile:manage'), async (req, res) => {
  await db.deleteOne('snmp_profiles', { id: db.id(req.params.id) });
  res.json({ message: 'Deleted' });
});

// ── Settings ──
router.get('/settings', requireAuth, requirePerm('pages', 'settings'), async (req, res) => {
  const pollerEngine = require('../pollers/poller-engine');
  const notify = {};
  ...
  res.json({
    snmp_interval_ms: await pollerEngine.effSnmpMs(),
    ping_interval_ms: await pollerEngine.effPingMs(),
    retention_days: await db.getSetting('retention_days', 365),
    aggregate_after_days: await db.getSetting('aggregate_after_days', 30),
    notify
  });
});

router.put('/settings', requireAuth, requirePerm('ops', 'settings:save'), async (req, res) => {
  ...
  if (b.retention_days !== undefined || b.aggregate_after_days !== undefined) {
    const parseDay = (v, lo, hi) => { const n = parseInt(v); return (isNaN(n) || n < lo || n > hi) ? null : n; };
    if (b.retention_days !== undefined) {
      const v = parseDay(b.retention_days, 1, 3650);
      if (v === null) return res.status(400).json({ error: 'retention_days must be 1-3650' });
      await db.setSetting('retention_days', v);
    }
    if (b.aggregate_after_days !== undefined) {
      const v = parseDay(b.aggregate_after_days, 1, 365);
      if (v === null) return res.status(400).json({ error: 'aggregate_after_days must be 1-365' });
      await db.setSetting('aggregate_after_days', v);
    }
    const maintenance = require('../database/maintenance');
    await maintenance.runMaintenance().catch(() => {});
  }
  const pollerEngine = require('../pollers/poller-engine');
...
// ── Events ──
router.get('/events', requireAuth, requirePerm('pages', 'events'), async (req, res) => {
  const q = req.query || {};
  let filter = {};
  if (q.device_id) filter.device_id = db.id(q.device_id);
...
// ── Dashboard ──
router.get('/dashboard/stats', requireAuth, requirePerm('pages', 'dashboard'), async (req, res) => {
...
// ── Tools ──
router.get('/tools/ping/:target', requireAuth, requirePerm('ops', 'tool:run'), async (req, res) => {
  res.json(await pingPoller.pingHost(req.params.target));
});

router.get('/tools/traceroute/:target', requireAuth, requirePerm('ops', 'tool:run'), async (req, res) => {
  try { res.json(await tools.traceroute(req.params.target, parseInt(req.query.hops) || 30)); }
  catch (e) { res.status(500).json({ error: e.message }); }
});

router.get('/tools/mtr/:target', requireAuth, requirePerm('ops', 'tool:run'), async (req, res) => {
  try { res.json(await tools.mtr(req.params.target, parseInt(req.query.cycles) || 4)); }
  catch (e) { res.status(500).json({ error: e.message }); }
});

router.get('/tools/portscan/:target', requireAuth, requirePerm('ops', 'tool:run'), async (req, res) => {
...
router.get('/tools/dns/:target', requireAuth, requirePerm('ops', 'tool:run'), async (req, res) => {
...
router.get('/tools/http', requireAuth, requirePerm('ops', 'tool:run'), async (req, res) => {
  let u;
...
router.get('/tools/snmpwalk', requireAuth, requirePerm('ops', 'tool:run'), async (req, res) => {
...
// ── Discovery ──
router.post('/discovery/scan', requireAuth, requirePerm('ops', 'discovery:run'), async (req, res) => {
...
router.get('/discovery/jobs', requireAuth, requirePerm('pages', 'discovery'), async (req, res) => {
  res.json(await db.find('discovery_jobs', {}, { sort: { created_at: -1 }, limit: 50 }));
});

// ── Poll Trigger ──
router.post('/trigger-poll', requireAuth, requirePerm('ops', 'trigger:poll'), async (req, res) => {
  const pollerEngine = require('../pollers/poller-engine');
